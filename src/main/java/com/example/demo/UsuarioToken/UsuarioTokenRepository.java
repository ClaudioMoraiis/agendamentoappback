...
public interface UsuarioTokenRepository extends JpaRepository<UsuarioTokenVO, Long> {
    ...
    @Query("SELECT ut.token from UsuarioTokenVO ut " +
    ...
    @Query("SELECT u FROM UsuarioTokenVO ut " +
            "  INNER JOIN ut.usuario u " +
            "WHERE (u.email = :mEmail)")
    ...
    @Query("""
       ...
       WHERE u.email = :mEmail
         AND ut.dthrExpiracao > :mDataHora
       """)
    String buscarTokenValido(
            @Param("mEmail") String email,
            @Param("mDataHora") LocalDateTime dataHora);
